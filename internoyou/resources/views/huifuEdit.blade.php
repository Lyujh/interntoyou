...
@section('content')
 <DIV>
    ...
    <DIV>
        <FORM onsubmit="return check()" method="post" name="postForm" action="{{ url('doHuifuEdit') }}">
            @csrf
            <DIV class="t">
                <TABLE cellSpacing="0" cellPadding="0" align="center">
                    <TBODY>
                    <TR>
                        <TD class="h" colSpan="3"><B>Modify reply</B></TD>
                    </TR>
                    <INPUT type="hidden" name="tie_id" value="{{ $tie_id }}">
                    <INPUT type="hidden" name="bankuai_id" value="{{ $bankuai->bankuai_id }}">
                    <INPUT type="hidden" name="huifu_id" value="{{ $huifu->huifu_id }}">
                    ...
                    <TR class="tr3">
                        ...
                        <TH colSpan="2">
                            <DIV>
                                <SPAN>
                                    <TEXTAREA style="WIDTH: 500px" tabIndex="2" rows="20" cols="90" name="content">{{ $huifu->content }}</TEXTAREA>
